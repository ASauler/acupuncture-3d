...
class GroupedLinkedMap<K extends Poolable, V> {
    ...
    private static class LinkedEntry<K, V> {
        ...
        public V removeLast() {
            int size = size();
            if (size > 0) {
                return this.f167values.remove(size - 1);
            }
            return null;
        }

        public int size() {
            List<V> list = this.f167values;
            if (list != null) {
                return list.size();
            }
            return 0;
        ...
        public void add(V v) {
            if (this.f167values == null) {
                this.f167values = new ArrayList();
            }
            this.f167values.add(v);
        }
    }
}
