...
class ClockFaceView extends RadialViewGroup implements ClockHandView.OnRotateListener {
    ...
    private void updateTextViews(int i) {
        ...
        for (int i2 = 0; i2 < Math.max(this.f178values.length, size); i2++) {
            TextView textView = this.textViewPool.get(i2);
            if (i2 >= this.f178values.length) {
                removeView(textView);
                ...
                textView.setText(this.f178values[i2]);
                textView.setTag(R.id.material_value_index, Integer.valueOf(i2));
                ViewCompat.setAccessibilityDelegate(textView, this.valueAccessibilityDelegate);
                textView.setTextColor(this.textColor);
                if (i != 0) {
                    textView.setContentDescription(getResources().getString(i, this.f178values[i2]));
                }
            }
        }
    }
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo accessibilityNodeInfo) {
        ...
        AccessibilityNodeInfoCompat.wrap(accessibilityNodeInfo).setCollectionInfo(AccessibilityNodeInfoCompat.CollectionInfoCompat.obtain(1, this.f178values.length, false, 1));
    }
