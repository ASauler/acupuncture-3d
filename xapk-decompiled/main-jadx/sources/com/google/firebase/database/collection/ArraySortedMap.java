...
public class ArraySortedMap<K, V> extends ImmutableSortedMap<K, V> {
    ...
    public V get(K k) {
        ...
        if (iFindKey != -1) {
            return this.f256values[iFindKey];
        }
        return null;
    }
    ...
    public ImmutableSortedMap<K, V> remove(K k) {
        ...
        return new ArraySortedMap(this.comparator, removeFromArray(this.keys, iFindKey), removeFromArray(this.f256values, iFindKey));
    }
    ...
    public ImmutableSortedMap<K, V> insert(K k, V v) {
        ...
        if (iFindKey != -1) {
            K[] kArr = this.keys;
            if (kArr[iFindKey] == k && this.f256values[iFindKey] == v) {
                return this;
            }
            return new ArraySortedMap(this.comparator, replaceInArray(kArr, iFindKey, k), replaceInArray(this.f256values, iFindKey, v));
        }
        if (this.keys.length > 25) {
            ...
            while (true) {
                K[] kArr2 = this.keys;
                if (i < kArr2.length) {
                    map.put(kArr2[i], this.f256values[i]);
                    i++;
                } else {
                    map.put(k, v);
            ...
            return new ArraySortedMap(this.comparator, addToArray(this.keys, iFindKeyOrInsertPosition, k), addToArray(this.f256values, iFindKeyOrInsertPosition, v));
        }
    }
    ...
    public void inOrderTraversal(LLRBNode.NodeVisitor<K, V> nodeVisitor) {
        ...
        while (true) {
            ...
            nodeVisitor.visitEntry(kArr[i], this.f256values[i]);
            i++;
        }
    }
    ...
    private Iterator<Map.Entry<K, V>> iterator(int i, boolean z) {
        return new Iterator<Map.Entry<K, V>>(i, z) { // from class: com.google.firebase.database.collection.ArraySortedMap.1
            ...
            public Map.Entry<K, V> next() {
                ...
                Object[] objArr = ArraySortedMap.this.f256values;
