...
class SSOAuthHandler extends AuthHandler {
    ...
    SSOAuthHandler(TwitterAuthConfig twitterAuthConfig, Callback<TwitterSession> callback, int i) {
    ...
    private boolean startAuthActivityForResult(Activity activity) {
        String strAvailableSSOPackage = availableSSOPackage(activity.getPackageManager());
        if (strAvailableSSOPackage == null) {
            Twitter.getLogger().e("Twitter", "SSO app signature check failed", null);
            return false;
        }
        ComponentName componentName = new ComponentName(strAvailableSSOPackage, SSO_CLASS_NAME);
