...
public class BitmapCropTask extends AsyncTask<Void, Void, Throwable> {
    ...
    private boolean crop() throws Throwable {
        ...
        if (zShouldCrop) {
            if (SdkVersionUtils.checkedAndroid_Q() && PictureMimeType.isContent(this.mImageInputPath)) {
                exifInterface = new ExifInterface(ArrayPoolProvide.getInstance().openInputStream(getContext().getContentResolver(), Uri.parse(this.mImageInputPath)));
            } else {
        ...
        if (SdkVersionUtils.checkedAndroid_Q() && PictureMimeType.isContent(this.mImageInputPath)) {
            PictureFileUtils.writeFileFromIS(ArrayPoolProvide.getInstance().openInputStream(getContext().getContentResolver(), Uri.parse(this.mImageInputPath)), new FileOutputStream(this.mImageOutputPath));
        } else {
    ...
    private void saveImage(Bitmap bitmap) {
        ...
        OutputStream contentResolverOpenOutputStream = null;
        try {
            contentResolverOpenOutputStream = PictureContentResolver.getContentResolverOpenOutputStream(context, Uri.fromFile(new File(this.mImageOutputPath)));
            if (bitmap.hasAlpha() && !this.mCompressFormat.equals(Bitmap.CompressFormat.PNG)) {
                this.mCompressFormat = Bitmap.CompressFormat.PNG;
            }
            bitmap.compress(this.mCompressFormat, this.mCompressQuality, contentResolverOpenOutputStream);
            bitmap.recycle();
        } finally {
            BitmapLoadUtils.close(contentResolverOpenOutputStream);
        }
    }
