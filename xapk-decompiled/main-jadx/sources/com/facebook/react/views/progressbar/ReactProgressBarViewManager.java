...
public class ReactProgressBarViewManager extends BaseViewManager<ProgressBarContainerView, ProgressBarShadowNode> implements AndroidProgressBarManagerInterface<ProgressBarContainerView> {
    ...
    public long measure(Context context, ReadableMap readableMap, ReadableMap readableMap2, ReadableMap readableMap3, float f, YogaMeasureMode yogaMeasureMode, float f2, YogaMeasureMode yogaMeasureMode2, float[] fArr) {
        Integer numValueOf = Integer.valueOf(getStyleFromString(readableMap2.getString(PROP_STYLE)));
        Pair<Integer, Integer> pairCreate = this.mMeasuredStyles.get(numValueOf);
        if (pairCreate == null) {
            ProgressBar progressBarCreateProgressBar = createProgressBar(context, numValueOf.intValue());
        ...
        return YogaMeasureOutput.make(PixelUtil.toDIPFromPixel(((Integer) pairCreate.first).intValue()), PixelUtil.toDIPFromPixel(((Integer) pairCreate.second).intValue()));
    }
}
