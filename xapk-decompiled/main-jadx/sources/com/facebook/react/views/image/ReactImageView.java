...
public class ReactImageView extends GenericDraweeView {
    ...
    private class TilePostprocessor extends BasePostprocessor {
        ...
        public CloseableReference<Bitmap> process(Bitmap bitmap, PlatformBitmapFactory platformBitmapFactory) {
            ...
            CloseableReference<Bitmap> closeableReferenceCreateBitmap = platformBitmapFactory.createBitmap(ReactImageView.this.getWidth(), ReactImageView.this.getHeight());
            try {
                new Canvas(closeableReferenceCreateBitmap.get()).drawRect(rect, paint);
                return closeableReferenceCreateBitmap.mo190clone();
            } finally {
    ...
    private static GenericDraweeHierarchy buildHierarchy(Context context) {
        ...
        return new GenericDraweeHierarchyBuilder(context.getResources()).setRoundingParams(roundingParamsFromCornersRadius).build();
    }
        ...
        if (!z) {
            ...
            this.mDownloadListener = new ReactImageDownloadListener<ImageInfo>() { // from class: com.facebook.react.views.image.ReactImageView.1
                ...
                public void onFailure(String str, Throwable th) {
                    eventDispatcherForReactTag.dispatchEvent(ImageLoadEvent.createErrorEvent(UIManagerHelper.getSurfaceId(ReactImageView.this), ReactImageView.this.getId(), th));
                }
            };
        }
        this.mIsDirty = true;
            ...
            if (!hasMultipleSources() || (getWidth() > 0 && getHeight() > 0)) {
                ...
                if (!zShouldResize || (getWidth() > 0 && getHeight() > 0)) {
                    if (!isTiled() || (getWidth() > 0 && getHeight() > 0)) {
                        ...
                        Postprocessor postprocessorFrom = MultiPostprocessor.from(linkedList);
                        ResizeOptions resizeOptions = zShouldResize ? new ResizeOptions(getWidth(), getHeight()) : null;
                        ReactNetworkImageRequest reactNetworkImageRequestFromBuilderWithHeaders = ReactNetworkImageRequest.fromBuilderWithHeaders(ImageRequestBuilder.newBuilderWithSource(this.mImageSource.getUri()).setPostprocessor(postprocessorFrom).setResizeOptions(resizeOptions).setAutoRotateEnabled(true).setProgressiveRenderingEnabled(this.mProgressiveRenderingEnabled), this.mHeaders);
                        ...
                        this.mDraweeControllerBuilder.setAutoPlayAnimations(true).setCallerContext(this.mCallerContext).setOldController(getController()).setImageRequest(reactNetworkImageRequestFromBuilderWithHeaders);
                        ImageSource imageSource2 = this.mCachedImageSource;
                        if (imageSource2 != null) {
                            this.mDraweeControllerBuilder.setLowResImageRequest(ImageRequestBuilder.newBuilderWithSource(imageSource2.getUri()).setPostprocessor(postprocessorFrom).setResizeOptions(resizeOptions).setAutoRotateEnabled(true).setProgressiveRenderingEnabled(this.mProgressiveRenderingEnabled).build());
                        }
