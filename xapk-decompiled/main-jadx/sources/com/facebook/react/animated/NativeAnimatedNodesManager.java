...
public class NativeAnimatedNodesManager implements EventDispatcherListener {
    ...
    private void stopAnimationsForNode(AnimatedNode animatedNode) {
        ...
        while (i < this.mActiveAnimations.size()) {
            ...
            if (animatedNode.equals(animationDriverValueAt.mAnimatedValue)) {
                if (animationDriverValueAt.mEndCallback != null) {
                    WritableMap writableMapCreateMap = Arguments.createMap();
                    writableMapCreateMap.putBoolean("finished", false);
                    writableMapCreateMap.putDouble("value", animationDriverValueAt.mAnimatedValue.mValue);
                    animationDriverValueAt.mEndCallback.invoke(writableMapCreateMap);
                } else if (this.mReactApplicationContext != null) {
                    WritableMap writableMapCreateMap2 = Arguments.createMap();
                    writableMapCreateMap2.putInt("animationId", animationDriverValueAt.mId);
                    writableMapCreateMap2.putBoolean("finished", false);
                    writableMapCreateMap2.putDouble("value", animationDriverValueAt.mAnimatedValue.mValue);
                    this.mReactApplicationContext.emitDeviceEvent("onNativeAnimatedModuleAnimationFinished", writableMapCreateMap2);
                }
    ...
    public void stopAnimation(int i) {
        for (int i2 = 0; i2 < this.mActiveAnimations.size(); i2++) {
            ...
            if (animationDriverValueAt.mId == i) {
                if (animationDriverValueAt.mEndCallback != null) {
                    WritableMap writableMapCreateMap = Arguments.createMap();
                    writableMapCreateMap.putBoolean("finished", false);
                    writableMapCreateMap.putDouble("value", animationDriverValueAt.mAnimatedValue.mValue);
    ...
    public void removeAnimatedEventFromView(int i, String str, int i2) {
        ...
        while (listIterator.hasNext()) {
            ...
            if (strNormalizeEventName.equals(next.mEventName) && i == next.mViewTag && i2 == next.mValueNode.mTag) {
                listIterator.remove();
                return;
            }
        }
    ...
    public void runUpdates(long j) {
        ...
        for (int i2 = 0; i2 < this.mActiveAnimations.size(); i2++) {
            ...
            this.mRunUpdateNodeList.add(animationDriverValueAt.mAnimatedValue);
            if (animationDriverValueAt.mHasFinished) {
                z = true;
            }
        }
        updateNodes(this.mRunUpdateNodeList);
        ...
        if (z) {
            for (int size = this.mActiveAnimations.size() - 1; size >= 0; size--) {
                ...
                if (animationDriverValueAt2.mHasFinished) {
                    if (animationDriverValueAt2.mEndCallback != null) {
                        WritableMap writableMapCreateMap = Arguments.createMap();
                        writableMapCreateMap.putBoolean("finished", true);
                        writableMapCreateMap.putDouble("value", animationDriverValueAt2.mAnimatedValue.mValue);
                        animationDriverValueAt2.mEndCallback.invoke(writableMapCreateMap);
                    } else if (this.mReactApplicationContext != null) {
                        WritableMap writableMapCreateMap2 = Arguments.createMap();
                        writableMapCreateMap2.putInt("animationId", animationDriverValueAt2.mId);
                        writableMapCreateMap2.putBoolean("finished", true);
                        writableMapCreateMap2.putDouble("value", animationDriverValueAt2.mAnimatedValue.mValue);
                        this.mReactApplicationContext.emitDeviceEvent("onNativeAnimatedModuleAnimationFinished", writableMapCreateMap2);
                    }
