...
public class DevServerHelper {
    ...
    private static String getSHA256(String str) {
        try {
            ...
            messageDigest.reset();
            try {
                byte[] bArrDigest = messageDigest.digest(str.getBytes(Key.STRING_CHARSET_NAME));
    ...
    private String createBundleURL(String str, BundleType bundleType, String str2, boolean z, boolean z2) {
        ...
        objArr[7] = z ? ServerProtocol.DIALOG_RETURN_SCOPES_TRUE : Constants.CASEFIRST_FALSE;
        if (!z2) {
    ...
    public void openDebugger(final ReactContext reactContext, final String str) {
        this.mClient.newCall(new Request.Builder().url(String.format(Locale.US, "http://%s/open-debugger?appId=%s&device=%s", this.mPackagerConnectionSettings.getInspectorServerHost(), Uri.encode(this.mPackageName), Uri.encode(getInspectorDeviceId()))).method("POST", RequestBody.create((MediaType) null, "")).build()).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.6
            @Override // okhttp3.Callback
            public void onResponse(Call call, Response response) {
            }
