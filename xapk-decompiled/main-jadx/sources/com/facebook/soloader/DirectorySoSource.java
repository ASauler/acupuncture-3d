...
public class DirectorySoSource extends SoSource {
    ...
    protected int loadLibraryFrom(String str, int i, File file, StrictMode.ThreadPolicy threadPolicy) throws IOException {
        ...
        boolean zEquals = soFileByName.getName().equals(str);
        ElfByteChannel channel = null;
        if (z || !zEquals) {
            try {
                channel = getChannel(soFileByName);
            } catch (Throwable th) {
                if (channel != null) {
                    channel.close();
                }
        ...
        if (z) {
            loadDependencies(str, channel, i, threadPolicy);
        } else {
        ...
        try {
            if (zEquals) {
                ...
                SoLoader.sSoFileLoader.loadBytes(soFileByName.getAbsolutePath(), channel, i);
            }
            if (channel != null) {
                channel.close();
            }
