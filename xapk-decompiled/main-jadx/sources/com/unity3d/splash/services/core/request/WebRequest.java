...
public class WebRequest {
    ...
    public long makeStreamRequest(OutputStream outputStream) throws Exception {
        ...
        if (getRequestType().equals(RequestType.POST.name())) {
            ...
            try {
                try {
                    printWriter = new PrintWriter((Writer) new OutputStreamWriter(httpUrlConnectionWithHeaders.getOutputStream(), Key.STRING_CHARSET_NAME), true);
                } catch (IOException e) {
                    e = e;
                }
            } catch (Throwable th) {
            ...
            try {
                printWriter.print(getBody() == null ? getQuery() : getBody());
                printWriter.flush();
                try {
                    printWriter.close();
                } catch (Exception e2) {
