...
final class InProcessTransport implements ServerTransport, ConnectionClientTransport {
    ...
    public synchronized ClientStream newStream(MethodDescriptor<?, ?> methodDescriptor, Metadata metadata, CallOptions callOptions, ClientStreamTracer[] clientStreamTracerArr) {
        ...
        if (this.serverMaxInboundMetadataSize != Integer.MAX_VALUE && (iMetadataSize = metadataSize(metadata)) > this.serverMaxInboundMetadataSize) {
            return failedClientStream(statsTraceContextNewClientContext, Status.RESOURCE_EXHAUSTED.withDescription(String.format("Request metadata larger than %d: %d", Integer.valueOf(this.serverMaxInboundMetadataSize), Integer.valueOf(iMetadataSize))));
        }
    ...
    public static int metadataSize(Metadata metadata) {
        ...
        for (int i = 0; i < bArrSerialize.length; i += 2) {
            length += (long) (bArrSerialize[i].length + 32 + bArrSerialize[i + 1].length);
        }
    ...
    private class InProcessStream {
        ...
        private InProcessStream(MethodDescriptor<?, ?> methodDescriptor, Metadata metadata, CallOptions callOptions, String str, StatsTraceContext statsTraceContext) {
            ...
            this.headers = (Metadata) Preconditions.checkNotNull(metadata, SdkBackendRequestLabels.HEADERS);
        ...
        private class InProcessServerStream implements ServerStream {
            ...
            public synchronized boolean clientRequested(int i) {
                ...
                if (this.clientReceiveQueue.isEmpty() && this.clientNotifyStatus != null) {
                    ...
                    this.clientStreamListener.closed(this.clientNotifyStatus, ClientStreamListener.RpcProgress.PROCESSED, this.clientNotifyTrailers);
                }
            ...
            private void notifyClientClose(Status status, Metadata metadata) {
                ...
                synchronized (this) {
                    ...
                    if (this.clientReceiveQueue.isEmpty()) {
                        ...
                        this.clientStreamListener.closed(statusCleanStatus, ClientStreamListener.RpcProgress.PROCESSED, metadata);
                    } else {
